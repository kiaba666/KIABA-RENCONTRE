...
# Vérifier si on est sur Render (présence de RENDER_EXTERNAL_URL)
...
# IMPORTANT: Toujours forcer SMTP en production, même si DEBUG est True par erreur
if not DEBUG or os.environ.get("FORCE_SMTP", "False").lower() in ("true", "1"):
    ...
    # Logging pour debug au démarrage
    import logging

    email_logger = logging.getLogger("django.mail")
    email_logger.setLevel(logging.INFO)
    email_logger.info(
        f"📧 EMAIL CONFIG: BACKEND={EMAIL_BACKEND}, HOST={EMAIL_HOST}, PORT={EMAIL_PORT}, SSL={EMAIL_USE_SSL}, TLS={EMAIL_USE_TLS}, USER={EMAIL_HOST_USER[:3] if EMAIL_HOST_USER else 'None'}***, DEBUG={DEBUG}"
    )

    # Vérifier que le backend n'est pas console
    if "console" in EMAIL_BACKEND.lower():
        email_logger.error("⚠️ ATTENTION: Le backend console est utilisé au lieu de SMTP!")
        EMAIL_BACKEND = "django.core.mail.backends.smtp.EmailBackend"
        email_logger.info(f"✅ Backend forcé à SMTP: {EMAIL_BACKEND}")

# Configuration avancée pour les emails
# S'assurer que le nom "KIABA" apparaît dans les emails
...
# Configuration SMTP avancée pour meilleure délivrabilité et éviter les spams
EMAIL_USE_LOCALTIME = True
EMAIL_SUBJECT_PREFIX = "[KIABA] "

# Configuration pour améliorer la délivrabilité et éviter les spams
...
# S'assurer que les emails sont bien formatés
